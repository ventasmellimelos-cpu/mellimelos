    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        {[
          ...
          { label: "Producto más caro", value: stats.topProduct?.name?.slice(0, 15) + "..." || "-", icon: TrendingUp, color: "bg-[#E8E0F0]", iconColor: "text-[#B8A8D8]" },
        ].map((stat) => {
      ...
      <div className="bg-white rounded-2xl shadow-soft overflow-hidden">
        ...
        {recentProducts.length === 0 ? (
          <div className="p-8 text-center">
            ...
            <p className="font-body text-[#6B6B6B]">No hay productos todavía</p>
          </div>
        ) : (
            ...
            {recentProducts.map((p) => (
              <div key={p.id} className="flex items-center gap-4 p-4 hover:bg-[#FFF8F0] transition-colors">
                <img
                  src={p.imageUrl ?? ""}
                  alt={p.name}
                  className="w-12 h-12 rounded-lg object-cover bg-[#F5F0EB]"
                />
                <div className="flex-1 min-w-0">
                  <p className="font-body font-medium text-sm text-[#2D2D2D] truncate">
                    {p.name}
                  </p>
                ...
                <div className="text-right">
                  ...
                  {p.salePrice && (
                    <p className="font-body text-xs text-[#D4A5A5]">
                      ${parseFloat(p.salePrice).toLocaleString("es-AR")}
                    </p>
                  )}
                </div>
